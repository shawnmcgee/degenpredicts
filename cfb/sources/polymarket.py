"""Polymarket event-contract prices for college football.
...
0.07, maker 0.00), so everything downstream of the ask is shared - see :mod:`venues`.
    ...
    >>> python -m cfb.sources.polymarket --discover
...
so the same ``sources/odds.py`` matcher reduces them to CFBD names.
"""
...
from core.settings import HTTP_TIMEOUT
...
TAG = os.environ.get("DEGEN_POLYMARKET_CFB_TAG", "cfb")
...
def fetch_markets(tag: str | None = None, closed: bool = False,
    ...
    if not out:
        log.warning("polymarket: no markets for tag %r. Confirm it with "
                    "`python -m cfb.sources.polymarket --discover`.", tag)
    else:
...
def _board(kind: str | None, matcher=None) -> pd.DataFrame:
    ...
    if not rows:
        log.warning("polymarket: %d %s markets returned but none parsed - the shapes have "
                    "changed; run `python -m cfb.sources.polymarket --discover`.",
...
def _discover() -> None:
    ...
    if not markets:
        print("\n!! Nothing returned. Try another tag, e.g.:\n"
              "     DEGEN_POLYMARKET_CFB_TAG=college-football python -m cfb.sources.polymarket --discover\n"
