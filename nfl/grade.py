"""Grade published NFL picks and write metrics.json.

    python -m nfl.grade

Grading joins on nflverse's ``game_id`` (``2026_01_DAL_NYG``), so there is no team-name
matching to go wrong. Closing-line value is measured against the number nflverse records after
the game, which for the NFL really is the closing line rather than a mid-week snapshot - that
makes CLV here a genuinely sharp instrument, and it is the metric to watch. A win rate needs
hundreds of games before it says anything; CLV says something within weeks.
"""
...
def grade() -> pd.DataFrame:
    ...
    if m.empty:
        ...
        return done
    # the grading below adds a dozen columns one at a time; copy once so pandas does not
    # warn about a fragmented frame on every run
    m = m.copy()
...
def metrics(done: pd.DataFrame) -> dict:
    ...
    out = {"updated": str(today), "sport": "nfl", "break_even": round(BREAK_EVEN, 2),
           "venue": config.VENUE, "totals": {}, "spreads": {}, "by_week": []}
    if done.empty:
        return out
    ...
    for kind, edge_col, err_col, strength_col in (
        ...
        out[key] = {
            ...
            # `all_games` is the one to read early. With 272 games a season and thresholds set
            # above every disagreement bucket that cleared break-even, `season` will often be
            # empty by design - but every game is still predicted, graded and CLV-tracked, so
            # the evidence accumulates whether or not anything is staked.
    ...
    for row in out["by_week"]:
        cum_t += row["total_units"]
        cum_s += row["spread_units"]
        row["cum_total"] = round(cum_t, 2)
        row["cum_spread"] = round(cum_s, 2)
    return out
